...
BACKUP_DIR="/opt/clinic-app-data.backup-batch127-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch127-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch127-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch127-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch127-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# دفعة 127 بتستبدل شجرة الكود كلها. السيرفر دلوقتي المفروض يكون على Batch 126،
# فبنقارن بملفات 126 للملفات اللي الدفعة دي بتغيّرها بس — أي تعديل يدوي على
# السيرفر فيها بيوقف النشر (نفس منطق فحص batch123→126 اللي كان في deploy126.sh).
#
# استثناء واحد متعمّد: src/app/patients/[id]/orthodontics/page.tsx. الملف ده
# معروف إنه مش متطابق على اللايف مع أرشيف batch126 (سبب النشر اللي فات لسه مش
# معروف بالظبط)، وده بالظبط الـbug اللي الدفعة دي جاية تصلحه — فالفحص الصارم
# (fail) هيقفل النشر للأبد على نفس المشكلة اللي بنيجي نصلحها. الملف ده بياخد
# فحص منفصل تحت (تحذير بس، مش إيقاف)، وخطوة "Batch127 post-swap verification"
# في الآخر هي اللي هتتأكد فعليًا إنه اتنشر صح المرة دي.
echo "== checking live batch-126 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 126: $relative. Merge the server-side change before deploying batch 127."
  fi
done <<'BATCH126_BASELINE_HASHES'
62447e43b468bb805c642b4eee15e40c107e603124260a8d97eb4481fb812ad6 src/app/api/patient-files/upload/route.ts
9e949ffd9d5793951513aa82d5c1c2c478c790deb6f1e50da390641c0ccc7bd5 src/app/patients/[id]/billing/page.tsx
20293fdd67ba18190b00715a3a55dd89af33bb178434de860eaafadf33cd800a src/app/patients/[id]/images/template-actions.ts
fb48fe0d1669da0eb0a630a174e5d70212e06f3187b6f56e86b35718258b81a3 src/components/AddImageGroupForm.tsx
eefc5d8d9f177851ed10fd017cc8e563fb429b95520cf69d4fd9d45a3031ef9a src/components/PatientAvatar.tsx
c569740f85ac00b67be01a4a3f3489d8e551244ea68480d04609e62e6c19b864 src/components/ortho/ImageTemplateBoard.tsx
c699c6d195a38eabe3a0fdde4acff0079d69dd3d4c123df6ff7cba5881c11d95 src/components/ortho/OrthoAddImageGroupForm.tsx
b952daea42b184d51c10ea1e8177e8ced7e89ba33634d7a259a0c0dce9a4497b src/lib/db/client.ts
29312624faa4223dd1c8d31ecc811bf983920c0ab4280848d9a605427bb7f359 src/lib/db/schema.sql
35500e4fb4eb058e3b66fa3c01bdea3deedce165c73fa71929034b044c96761e src/lib/db/types.ts
1ebfa7cc525a3bbfc10937e0550b43e61f8ae8f741d813b6d522e1fefea009c7 src/lib/images/classify.ts
38f4e288423822097fbabca3c1ee49fa5e71038297f621d254ad24fe1aeb1096 src/lib/images/kinds.ts
b28806fe3f0bb01d712688366b5a8ee2fff7a25f8f4c88939e7d65e0c6907eff src/lib/images/order.ts
65800a78fdc369249171544842b6f225cf61af072f43c026f4600ade5d7f7dc9 src/lib/patient-file-upload.ts
BATCH126_BASELINE_HASHES

# الملف المعروف بمشكلة النشر السابقة — تحذير بس، مش إيقاف (اتفقنا: يتصلح رغم
# أي حاجة). لو مطابق فعلاً، يبقى اللغز اتحل لوحده والملف صح أصلاً على اللايف.
ORTHO_PAGE_REL="src/app/patients/[id]/orthodontics/page.tsx"
ORTHO_PAGE_LIVE="$APP_DIR/$ORTHO_PAGE_REL"
ORTHO_PAGE_EXPECTED_126="f6336804b3b52cb6ef42d9c21f8a54be9f90ad593e78b424d581ed130febd2dd"
if [[ -f "$ORTHO_PAGE_LIVE" ]]; then
  ortho_actual="$(sha256sum "$ORTHO_PAGE_LIVE" | awk '{print tolower($1)}')"
  if [[ "$ortho_actual" != "$ORTHO_PAGE_EXPECTED_126" ]]; then
    echo "[deploy127] NOTE: live $ORTHO_PAGE_REL does not match the batch126 archive (this is the known stale-file bug being fixed now — proceeding, will be overwritten and verified post-swap)." >&2
  else
    echo "[deploy127] $ORTHO_PAGE_REL matches the batch126 archive on the live server."
  fi
else
  echo "[deploy127] NOTE: live $ORTHO_PAGE_REL is missing entirely." >&2
fi
...
grep -q "function fileSha256" "$STAGE_DIR/src/app/api/patient-files/upload/route.ts" \
...
grep -q "lightboxId" "$STAGE_DIR/src/components/ortho/ImageTemplateBoard.tsx" \
...
LIVE_HASHES_FILE="$APP_DIR/.deploy127-expected-hashes"
[[ -f "$LIVE_HASHES_FILE" ]] || fail "Post-swap verification snapshot did not survive the swap (.deploy127-expected-hashes missing) — aborting rather than guessing."
...
while IFS= read -r line; do
  ...
  if [[ ! -f "$live_file" ]]; then
    echo "[deploy127] MISMATCH: expected file missing after swap: $rel" >&2
    POST_SWAP_MISMATCH=1
    continue
  fi
  live_hash="$(sha256sum "$live_file" | cut -d' ' -f1)"
  if [[ "$live_hash" != "$expected_hash" ]]; then
    echo "[deploy127] MISMATCH: live file does not match what was just built/tested: $rel" >&2
    POST_SWAP_MISMATCH=1
  fi
done < "$LIVE_HASHES_FILE"
rm -f "$LIVE_HASHES_FILE"
if (( POST_SWAP_MISMATCH != 0 )); then
  fail "Post-swap verification failed — the live app does not match the batch127 archive. Rolling back automatically instead of serving unverified files."
fi
...
echo
echo "== Batch 127 =="
echo "Batch 127: gallery ordering fix verified live, financial-page patient photo, Profile/45° smiling kinds,"
