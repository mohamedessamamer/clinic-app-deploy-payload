...
ARCHIVE="clinic-app-batch106.tar.gz"
ARCHIVE_SHA256="5CB1E8122A30D42D062AA90414499DC66B32CF8667E8A2DFF4153D9B09224615"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch106-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch106-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch106-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch106-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch106-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# --------------------------------------------------------------------------
# Batch 106: edit-billing-entry, patients pagination/sort/live search, whole-
...
grep -q "edit_patient_billing_entry" "$STAGE_DIR/src/lib/permission-defs.ts" \
  || fail "Batch 106: the edit_patient_billing_entry permission is missing from permission-defs.ts."
grep -q "editPatientBillingEntryAction" "$STAGE_DIR/src/app/patients/[id]/billing/actions.ts" \
  || fail "Batch 106: editPatientBillingEntryAction is missing from patients/[id]/billing/actions.ts."
grep -q "edit_billing_entry" "$STAGE_DIR/src/app/patients/[id]/billing/actions.ts" \
  || fail "Batch 106: the edit_billing_entry audit event is missing from patients/[id]/billing/actions.ts."
grep -q "EditInvoiceButton" "$STAGE_DIR/src/app/patients/[id]/billing/page.tsx" \
  || fail "Batch 106: EditInvoiceButton is not wired into patients/[id]/billing/page.tsx."
...
grep -q "PAGE_SIZE = 100" "$STAGE_DIR/src/app/patients/page.tsx" \
  || fail "Batch 106: patients page size is not 100."
grep -q "PatientsSearchBar" "$STAGE_DIR/src/app/patients/page.tsx" \
  || fail "Batch 106: the live search bar is not wired into src/app/patients/page.tsx."
...
grep -q "searchPatientsForBookingAction" "$STAGE_DIR/src/app/appointments/actions.ts" \
  || fail "Batch 106: searchPatientsForBookingAction is missing from appointments/actions.ts."
grep -q "searchPatientsForBookingAction" "$STAGE_DIR/src/components/CentralSchedule.tsx" \
  || fail "Batch 106: CentralSchedule.tsx does not call searchPatientsForBookingAction - the booking search is still capped."
...
grep -q "inline-block" "$STAGE_DIR/src/components/CentralSchedule.tsx" \
  || fail "Batch 106: the patient-name link click-area fix (inline-block) is missing from CentralSchedule.tsx."

# 5) Low-stock alert recipients moved to a permission.
grep -q "receive_low_stock_alerts" "$STAGE_DIR/src/lib/permission-defs.ts" \
  || fail "Batch 106: the receive_low_stock_alerts permission is missing from permission-defs.ts."
grep -q "getUsersWithPermission" "$STAGE_DIR/src/lib/permissions.ts" \
  || fail "Batch 106: getUsersWithPermission helper is missing from permissions.ts."
if grep -q "inventory_low_stock_recipient_ids" "$STAGE_DIR/src/components/InventoryCatalogManager.tsx"; then
  fail "Batch 106: InventoryCatalogManager.tsx still references the old recipient-list setting."
fi

# 6) Suppliers under Expenses: schema, permission, CRUD, invoices/payments.
require_file "$STAGE_DIR/src/components/InventorySupplierManager.tsx"
require_file "$STAGE_DIR/src/app/expenses/suppliers/page.tsx"
grep -q "manage_suppliers" "$STAGE_DIR/src/lib/permission-defs.ts" \
  || fail "Batch 106: the manage_suppliers permission is missing from permission-defs.ts."
grep -q "106_supplier_invoices_payments" "$STAGE_DIR/src/lib/db/client.ts" \
  || fail "Batch 106: the supplier invoices/payments migration is missing from client.ts."
grep -q "CREATE TABLE IF NOT EXISTS supplier_invoices" "$STAGE_DIR/src/lib/db/client.ts" \
  || fail "Batch 106: the supplier_invoices table definition is missing from client.ts."
grep -q "CREATE TABLE IF NOT EXISTS supplier_payments" "$STAGE_DIR/src/lib/db/client.ts" \
  || fail "Batch 106: the supplier_payments table definition is missing from client.ts."
grep -q "addSupplierInvoiceAction" "$STAGE_DIR/src/app/expenses/actions.ts" \
  || fail "Batch 106: addSupplierInvoiceAction is missing from expenses/actions.ts."
grep -q "recordSupplierInvoicePaymentAction" "$STAGE_DIR/src/app/expenses/actions.ts" \
  || fail "Batch 106: recordSupplierInvoicePaymentAction is missing from expenses/actions.ts."
grep -q "recordSupplierGeneralPaymentAction" "$STAGE_DIR/src/app/expenses/actions.ts" \
  || fail "Batch 106: recordSupplierGeneralPaymentAction is missing from expenses/actions.ts."
# Every recorded supplier payment (invoice-linked or general) must post an
# expenses row under category "suppliers" - checked by intent (both action
# bodies must reference the suppliers expense category), not by a single
# string, since the whole point of the item-6 fix was that the invoice-linked
# path used to skip this.
awk '/^export async function recordSupplierInvoicePaymentAction/{f=1} f{print} f&&/^}/{exit}' \
  ...
  || fail "Batch 106: recordSupplierInvoicePaymentAction does not record an expenses row (category suppliers) - invoice-linked supplier payments would be missing from expense totals."
awk '/^export async function recordSupplierGeneralPaymentAction/{f=1} f{print} f&&/^}/{exit}' \
    "$STAGE_DIR/src/app/expenses/actions.ts" | grep -q 'category: "suppliers"' \
  || fail "Batch 106: recordSupplierGeneralPaymentAction does not record an expenses row (category suppliers)."
grep -q "is_ortho" "$STAGE_DIR/src/lib/db/types.ts" || fail "Batch 106: is_ortho column missing from db types."
grep -q "is_general" "$STAGE_DIR/src/lib/db/types.ts" || fail "Batch 106: is_general column missing from db types."

# 7) Expenses page: new summary boxes, old boxes removed.
if grep -q "المحصل فعليا" "$STAGE_DIR/src/app/expenses/page.tsx"; then
  fail "Batch 106: 'المحصل فعليا' box should have been removed from expenses/page.tsx (moved to Reports)."
fi
if grep -q "صافي النقدية بعد المصروفات" "$STAGE_DIR/src/app/expenses/page.tsx"; then
  fail "Batch 106: 'صافي النقدية بعد المصروفات' box should have been removed from expenses/page.tsx (moved to Reports)."
fi
for label in "موردين" "معامل" "أخرى"; do
  grep -q "$label" "$STAGE_DIR/src/app/expenses/page.tsx" \
    || fail "Batch 106: the '$label' summary box is missing from expenses/page.tsx."
done
...
grep -q "getPayrollDetailReport" "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 106: the detailed payroll report is missing from reports/page.tsx."
grep -q "getSuppliersDetailReport" "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 106: the detailed suppliers report is missing from reports/page.tsx."
grep -q "getMiscExpensesDetailReport" "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 106: the detailed misc-expenses report is missing from reports/page.tsx."
grep -q "is_ortho" "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 106: the suppliers report is not split by is_ortho/is_general in reports/page.tsx."

# Every "use server" file must still export only async functions.
node "$STAGE_DIR/scripts/check-use-server.mjs" "$STAGE_DIR/src" \
  || fail "Batch 106: a \"use server\" file exports something other than an async function."
...
echo
echo "Batch 106 adds: edit button on patient billing rows; patients list"
echo "pagination/sorting/live search (100/page); whole-database booking"
echo "search (500-row cap removed); the إضافة خدمة vs. حضر/تم click-area"
echo "fix on the front-desk calendar; a receive_low_stock_alerts permission"
echo "(replacing the old inventory settings recipient list); a new Suppliers"
echo "(الموردين) feature under Expenses with invoices/payments and a debt"
echo "figure; new موردين/معامل/أخرى boxes on the Expenses page; and a"
echo "revenue/expenses split with new payroll, suppliers and misc-expense"
echo "detail reports under Financial Reports."
echo
