...
ARCHIVE="clinic-app-batch117.tar.gz"
ARCHIVE_SHA256="C2EC960F74CA264EB8D26A35B5FD98FBA9B7CA73F9CEA7BEFFEFF14D5735336A"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch117-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch117-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch117-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch117-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch117-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 117 replaces the full source tree. The server is already on Batch 116,
# so reject any hotfix made to files this batch updates.
echo "== checking live batch-116 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 116: $relative. Merge the server-side change before deploying batch 117."
  fi
done <<'BATCH117_BASELINE_HASHES'
8c9c94117a55ae9b72d6d7182958b590a69686351d9d8a6fa10263952647bfdf src/components/CentralSchedule.tsx
4c3b0c0c0190ff2026baba4dce27d2d9006b3b9b282669a3e53acfef93a49fa7 src/app/globals.css
8f267682ca0eac19865bb9dab78f04083e75eb6cef8dc0fdc8a9fdc768578137 src/app/layout.tsx
6dcf9dc0eb817d789ebb22864cc1ba45e4cf5cb4e0cb178ffa84ed43964ed596 src/app/page.tsx
BATCH117_BASELINE_HASHES
