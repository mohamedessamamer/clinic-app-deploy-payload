...
ARCHIVE="clinic-app-batch130.tar.gz"
ARCHIVE_SHA256="F0C87E19AA8BF8769EF401394C088CCBAF1E436B44084BD57AF69970A82A7274"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch130-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch130-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch130-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch130-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch130-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 130 upgrades the published Batch 129 (== the deploy that shipped
# rotation-guess fix, decoupled manual rotate, and the ETag/cache-bust fix).
...
# clinic-app-batch129.tar.gz (not hand-copied from an older script) —
# covering the whole ortho-image-feature surface (same set batch129 checked)
# plus every file batch130 itself touches (settings.ts, template.ts,
# CentralSchedule.tsx, front-desk/page.tsx, globals.css, settings/actions.ts,
# settings/page.tsx, OrthoImageTemplateForm.tsx). As with every previous
# batch, spot-check a couple of these against the live server yourself
...
# mean the live server is not actually running batch129 as published.
echo "== checking live batch-129 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from published batch 129: $relative. Merge the server-side change before deploying batch 130."
  fi
done <<'BATCH129_BASELINE_HASHES'
...
e51137e8e913b07808b8c06505292d7ad797276a9745bcbd8f792eeca5befa24 src/app/patients/[id]/images/template-actions.ts
...
e038de553f0e48862969eb50c3d73f3b0df5ec00a19abbe8cf4d7e43826a6f94 src/components/ortho/ImageTemplateBoard.tsx
...
0309b59b2ccb3bbd3d68c4a5d8bb956810dbaf65b25d763f7a745da716442b0d src/lib/settings.ts
8089b9d64886a7efd19e59e4893620ad30a14abb1caa76e26cefe1ac8648bc41 src/lib/images/template.ts
cab0c5e6f0bbea2c13743723807cd8ea67c718f58918f40d5d714bb568bc084b src/app/settings/actions.ts
482307132bc742865f77fe94daa1ec4c36337ae07338ae27423d1a9d6f821e12 src/app/settings/page.tsx
112427930dfa7ed211c768f4351d323882958ea251dd2363d5f2d3d8384d362c src/components/OrthoImageTemplateForm.tsx
2824e06d21c022f0981abe57cd63ca1ad980c5b88e381a1917f619ae501f8fcc src/app/front-desk/page.tsx
05295fb58508c1c3a48991b85e0f71da67733889ad927ea6d90ede6b54a69c47 src/components/CentralSchedule.tsx
a317f4a16f6a9a35f987e30a8f7ffce89c0e86937696a2a9410546cccbcd201d src/app/globals.css
BATCH129_BASELINE_HASHES
...
POST_SWAP_HASHES_FILE="$STAGE_DIR/.deploy130-expected-hashes"
...
LIVE_HASHES_FILE="$APP_DIR/.deploy130-expected-hashes"
[[ -f "$LIVE_HASHES_FILE" ]] || fail "Post-swap verification snapshot did not survive the swap (.deploy130-expected-hashes missing) — aborting rather than guessing."
...
while IFS= read -r line; do
  ...
  if [[ ! -f "$live_file" ]]; then
    echo "[deploy130] MISMATCH: expected file missing after swap: $rel" >&2
    POST_SWAP_MISMATCH=1
    continue
  fi
  live_hash="$(sha256sum "$live_file" | cut -d' ' -f1)"
  if [[ "$live_hash" != "$expected_hash" ]]; then
    echo "[deploy130] MISMATCH: live file does not match what was just built/tested: $rel" >&2
    POST_SWAP_MISMATCH=1
  fi
done < "$LIVE_HASHES_FILE"
rm -f "$LIVE_HASHES_FILE"
if (( POST_SWAP_MISMATCH != 0 )); then
  fail "Post-swap verification failed — the live app does not match the batch130 archive. Rolling back automatically instead of serving unverified files."
fi
...
echo
echo "== Batch 130 =="
echo "Batch 130: upgrades published Batch 129."
echo "1) Settings template editor split into sections + admin-defined custom image kinds (manual-only)."
echo "2) Patient avatar shown on the home/front-desk schedule cards."
echo "3) Fix for the avatar-crop crash (كان بيطلع 'تعذر تجهيز صورة الملف')."
echo "4) Opt-in conservative learning from manual photo-type corrections (off by default; two tiers -"
echo "   uncertain classifications, and confident-but-repeatedly-wrong ones, both admin-toggled in Settings)."
