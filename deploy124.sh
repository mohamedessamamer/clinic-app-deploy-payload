...
BACKUP_DIR="/opt/clinic-app-data.backup-batch124-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch124-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch124-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch124-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch124-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
