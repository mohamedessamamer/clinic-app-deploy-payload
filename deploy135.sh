...
ARCHIVE="clinic-app-batch135.tar.gz"
ARCHIVE_SHA256="2006512539F7FFD9B9FFE00388C2F165B5AC473EF4A04483E27E74A2C15095C2"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch135-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch135-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch135-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch135-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch135-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 135 upgrades the published Batch 134 (== the deploy that added the
# statistics tab and reworked the incomplete-checkup report). Refuse to
# overwrite unreviewed server-side changes before stopping the app.
#
# Batch 135 is a styling-only change: the revere-palette top bar
# (.clinic-reception-nav) gets a new background color/fabric texture, plus a
# lighter active-link highlight. It touches exactly one existing file
# (src/app/globals.css) and adds one new asset (public/textures/fabric-twill.png,
# checked separately below since it does not exist on the live server yet).
# Hash computed directly from the published clinic-app-batch134.tar.gz. A
# mismatch here means someone edited globals.css on the live server.
echo "== checking live batch-134 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from published batch 134: $relative. Merge the server-side change before deploying batch 135."
  fi
done <<'BATCH134_BASELINE_HASHES'
4a16a66cf6f261888a9a5f2c86d68ebdf94c8fb4e9d86448d9ef4d7cd25ecfdc src/app/globals.css
BATCH134_BASELINE_HASHES
...
POST_SWAP_VERIFY_FILES=(
  ...
  # Batch 135 addition — new asset the revere top-bar restyle depends on.
  "public/textures/fabric-twill.png"
)
...
# --------------------------------------------------------------------------
# Batch 135: revere-palette top bar (.clinic-reception-nav) restyle only —
# new background color + fabric texture + a lighter active-link highlight.
# No database migration. The medical palette override is a separate, more
# specific selector (body[data-color-theme="medical"] .clinic-reception-nav)
# further down in the same file and must be byte-for-byte unchanged.
# --------------------------------------------------------------------------
echo "== batch 135: revere top-bar restyle is present and medical palette is untouched =="
require_file "$STAGE_DIR/public/textures/fabric-twill.png"
require_file "$STAGE_DIR/docs/BATCH135.md"
grep -q 'background-color:#444331' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 135: the new top-bar color (#444331) is missing from globals.css."
grep -q 'url("/textures/fabric-twill.png")' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 135: the fabric-twill texture reference is missing from globals.css."
grep -q 'body\[data-color-theme="revere"\] \.clinic-reception-link:hover,' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 135: the revere-scoped active/hover link highlight is missing from globals.css."
# The medical override's own color values must be exactly what batch93 shipped
# (green gradient nav, #c7f0df active border) — this section must never touch
# them, so any drift here means the revere edit leaked into the medical rules.
grep -q 'body\[data-color-theme="medical"\] \.clinic-reception-nav,' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 135: the medical top-bar override rule is missing from globals.css."
grep -q 'background:linear-gradient(90deg,#064c41,#08745e)' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 135: the medical top-bar gradient changed — it must stay independent from the revere restyle."
grep -q 'border-bottom-color:#c7f0df' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 135: the medical active-link border color changed — it must stay independent from the revere restyle."
...
echo "== verifying Batch 112 through 134 database changes (batch 135 adds no schema change) =="
SUCCESS=0
...
echo "== Batch 135 =="
echo "Batch 135: styling only, no database migration."
echo "1) Revere-palette top bar (.clinic-reception-nav) recolored from brown (#614832) to a dark"
echo "   olive (#444331), with a fabric (twill weave) texture and a subtle 3D highlight/shadow."
echo "   Dimensions and font sizes are unchanged."
echo "2) The active/hovered top-bar link is now a bit lighter (rgba(255,255,255,.16) vs the old .09)"
echo "   so it still reads clearly against the darker background - revere palette only."
echo "3) Medical palette top bar is untouched (separate override, verified explicitly above)."
