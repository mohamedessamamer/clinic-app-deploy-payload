...
ARCHIVE="clinic-app-batch119.tar.gz"
ARCHIVE_SHA256="C8F7A49F57F7C81D53726DBFD4254E54E75536E51F0C849C0AD0211C47D577E2"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch119-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch119-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch119-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch119-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch119-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 119 replaces the full source tree. The server is already on Batch 118,
# so reject any hotfix made to files this batch updates.
echo "== checking live batch-118 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 118: $relative. Merge the server-side change before deploying batch 119."
  fi
done <<'BATCH119_BASELINE_HASHES'
fd657132ce3a92a5558092f716f3a8a22196648692111421c5e7ef63220630d7 src/app/page.tsx
b78d119d92a14ea9138dfba33e7812cb4ae80876817608d6e6221825646be4e2 src/app/globals.css
38db60bdafa6bd8b2f1a5916338b64101639c9e241f2540478329ec728ea979f src/components/CentralSchedule.tsx
99ac72fa19cb84458e3144d155aca9b34ab9d40226254f8c44d9d0822fcf1426 src/lib/settings.ts
e2f738b30102e7dbee426c803a8454801455a0dba98c26cc9686909a2429e3f6 src/app/settings/actions.ts
c14028550a04884e39f24c5bb54939f375a25965c711ef629e1c9bc70d7ef58c src/app/settings/page.tsx
219474d6a1c9711b171fd3a60823b1ca8925cd58269f314550aaf0c1571743ab src/components/StaffChat.tsx
cdf2434b754b58eb29948859f35b78261e0749e6c68e24da6463bad3de29f4f4 src/app/reports/patient-duration-actions.ts
d1763ff45c35e05af224d63343d5d77ca0d5f26c757a360b88304afc8d8d6ddd src/components/PatientDurationReport.tsx
BATCH119_BASELINE_HASHES
[[ ! -e "$APP_DIR/src/components/HomeAppointments.tsx" ]] || fail "Live source already contains HomeAppointments. Merge it before deploying batch 119."
...
echo "== verifying Batch 112 through 115 database changes =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 112-115 database verification failed."
...
if (!migration || setting?.value !== "0" || Number(tubeCells?.total) !== 8 || !orthoColumns.includes("diagnosis_summary") || !patientColumns.includes("archived_at") || !patientColumns.includes("archive_activity_at") || foreignKeys.length !== 0) process.exit(1);
NODE
SUCCESS=1
...
echo
echo "== Batch 115 =="
echo "1) أرشيف مستقل للمرضى: يخفيهم من القائمة العادية ويستبعد أي تواصل يبدأ"
echo "   من العيادة، مع بقاء البحث والحجز والعلاج متاحين وإعادة التفعيل تلقائيًا."
echo "2) مرضى بلا نشاط لسنتين يدخلون الأرشيف تلقائيًا؛ الأرشفة اليدوية متاحة"
echo "   لصاحب صلاحية قائمة المرضى مع تنبيه لا يمنع وجود رصيد أو خدمة."
echo "3) جرد الـTubes صار على صفين علوي وسفلي مطابقين لاتجاه الـBrackets،"
echo "   ومحرر الشارت نزل سطرًا مع تكبير خطّي الخيارات والكتابة."
echo
echo "== Batch 114 =="
echo "1) مربع ملخص الـInteractive chart انتقل للمساحة البيضاء أسفل الأسنان،"
echo "   بعرض قريب من الشارت، وارتفاع وحواف أخف وخط كتابة أكبر."
echo "2) أيقونة المتصفح القديمة استُبدلت بالشعار الذهبي الصغير على خلفية سوداء،"
echo "   مع رابط إصدار جديد كي لا يعرض Chrome المثلث القديم."
echo
echo "== Batch 113 =="
echo "1) شارت التقويم يفتح على Treatment، وتبويباته Chief complaint وTreatment"
echo "   وDiagnosis، وكل ملخص متزامن تلقائيًا مع الـDiagnostic chart."
echo "2) أسفل الأسنان قائمة خيارات متعددة ومربع كتابة لكل ملخص، مع حفظ تلقائي."
echo "3) إعدادات Orthodontic chart تضبط خيارات الملخصات بالترتيب، بصلاحية مستقلة"
echo "   للأدمن أو من تُمنح له صلاحية إدارة قوائم شارت التقويم."
echo
echo "== Batch 112 =="
echo "1) خصم المخزن متوقف افتراضيًا بعد النشر لكل المستهلكات والبراكتس"
echo "   والتيوبز، مع مفتاح مستقل بصلاحية وتأكيد وسجل تشغيل/إيقاف."
echo "2) الـTubes لها 8 خانات مستقلة للأسنان 16/17/26/27/36/37/46/47،"
echo "   مع حالة Tube 6 وحالة Tube 7 وRefill وجرد يدوي من غير شركات."
echo "3) Tasks مستقلة عن الإشعارات: للاستقبال كفريق أو لمستخدم محدد، مرتبطة"
echo "   بمريض أو عامة، مع تم التنفيذ ثم إغلاق صاحب المهمة وأرشيف مغلق."
echo "4) تحويلات الأطباء أصبحت Tasks وتغلق تلقائيًا عند إنهاء زيارة المريض"
echo "   مع الطبيب المحول إليه."
echo "5) الطبيب صاحب صلاحية تعديل المواعيد يقدر يحجز ويلغي ويعدل في شيفتاته"
echo "   حتى 90 يومًا، ومع عرض كل العيادات + تعديل المواعيد يقدر يعدل الكل."
echo
echo "== Batch 110 =="
echo "1) خطوط الشريط والجدول أكبر، وخانة حضر تتلون بالكامل مع ألوان مناسبة"
echo "   لباليتي Revere وMedical، مع بقاء أفاتار العيادة بحجمه الآمن."
echo "2) البحث السريع يشمل كل المرضى بلا حد 500، ويبحث برقم الهاتف بعد"
echo "   تطبيع صيغته ويعرض الرقم في النتائج."
echo "3) الطبيب يقدر يفتح جدوله حتى 90 يومًا للأمام و14 يومًا للخلف."
echo "4) ترتيب الحضور يختفي عن الطبيب لو مفيش له شيفت اليوم، إلا لو عنده"
echo "   صلاحية عرض كل العيادات."
echo "5) تعديل الملف المالي يحدد متابعة من الخدمة المختارة فعليًا، ويتيح ربط"
echo "   الصفوف القديمة اليتيمة يدويًا مع منع الربط الذاتي/غير الصالح."
echo "6) الملف الطبي فيه فلتر General/Ortho وفلتر دكتور، ويحسب الطبيب الأساسي"
echo "   أو المساعد ويعرض فقط الدكاترة المسجلين في زيارات المريض."
echo
echo "== Batch 109 =="
echo "1) New report 'الكشوفات غير المكتملة' في تبويب تقارير المرضى: مرضى"
echo "   عملوا كشف مدفوع ومفيش أي خدمة تانية اتسجلت لهم بعد كده. فترة قابلة"
echo "   للاختيار + زرار الأسبوع الماضي/الشهر الماضي."
echo "2) صلاحيات تقارير المرضى اتوحدت: متابعة تقويم متأخرة + التحويلات"
echo "   الطبية + مدة المريض في العيادة + الكشوفات غير المكتملة كلهم دلوقتي"
echo "   تحت صلاحية واحدة اسمها 'عرض تقارير المرضى'. تقييمات المرضى (Review)"
echo "   لسه ليها صلاحيتها المنفصلة زي ما هي من غير تغيير."
echo "   مهم: راجع صفحة الصلاحيات - أي مستخدم كان شايف واحد بس من التقارير"
echo "   التلاتة القديمة بقى شايف كل تقارير المرضى الأربعة مع بعض دلوقتي."
echo "3) قائمة المرضى: الاستثناء اللي كان بيخلي أي دور غير الطبيب يشوفها"
echo "   دايمًا اتشال. دلوقتي الأدمن بس هو اللي يشوفها افتراضيًا؛ أي دور تاني"
echo "   (دكتور/استقبال/محاسب/مدير عيادة) لازم تدّيله صلاحية 'قائمة المرضى'"
echo "   بنفسك من صفحة الصلاحيات لو عايزه يشوفها."
echo
echo "== Batch 108 (hotfix) =="
echo "Fixes a bug in batch 107's 'خامات' (suppliers) feature: any of إضافة"
echo "فاتورة / تسجيل دفعة / تعديل / حذف used to freeze the ENTIRE app for"
echo "every user (not just the one clicking the button), because the audit-"
echo "log call inside those actions ran on the wrong database connection"
echo "while a transaction on that same connection was still open - a"
echo "self-deadlock with no error and no timeout, recoverable only with a"
echo "manual 'systemctl restart clinic-app'. Fixed by moving every audit-log"
echo "call to run AFTER its transaction has fully closed. No other feature"
echo "or file is touched in this batch."
echo
