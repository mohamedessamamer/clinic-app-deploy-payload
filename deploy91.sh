...
ARCHIVE="clinic-app-batch91.tar.gz"
ARCHIVE_SHA256="4AFEBEAE324710BCDF79040D569C8C813069E1FB8D47A01E52DB75B8C72C4F46"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch91-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch91-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch91-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch91-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch91-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
if [[ "$ACTUAL_SHA256" != "$EXPECTED_SHA256" ]]; then
  fail "SHA256 mismatch on $ARCHIVE. The file on GitHub does not match the expected release (re-upload it, or this deploy91.sh is out of date)."
fi
echo "Checksum OK."
...
# Batch 91: real fix — a doctor added directly from HR (no login account) could
# never be deactivated before. Confirm the new action/button actually shipped.
if ! grep -q "endHrDoctorAction" "$STAGE_DIR/src/app/hr/actions.ts"; then
  fail "endHrDoctorAction is missing from src/app/hr/actions.ts — batch91 fix did not make it into this archive."
fi
if ! grep -q "endHrDoctorAction" "$STAGE_DIR/src/components/HrWorkspace.tsx"; then
  fail "endHrDoctorAction wiring is missing from src/components/HrWorkspace.tsx — batch91 fix did not make it into this archive."
fi
