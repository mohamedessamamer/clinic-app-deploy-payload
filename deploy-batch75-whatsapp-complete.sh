...
ARCHIVE="clinic-app-batch75-whatsapp-complete.tar.gz"
ARCHIVE_SHA256="C7DBA64D73AAF56EC273CCFA1F14C01E3F7231C2FEE55646CD3589338B0C45DD"
...
BACKUP_DIR="/opt/clinic-app-data.backup-whatsapp-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-whatsapp-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-whatsapp-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-whatsapp-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-whatsapp-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
