...
ARCHIVE="clinic-app-batch80.tar.gz"
ARCHIVE_SHA256="86D2717532C86A2BBCA925F4523C6290C6CB1D283526C88661EB55C9E801F96F"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch80-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch80-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch80-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch80-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch80-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
