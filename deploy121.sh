...
ARCHIVE="clinic-app-batch121.tar.gz"
ARCHIVE_SHA256="F3CF2706EA08E5696389E6C4827BBB523369297CC24BD522879B19AA27CDDDEB"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch121-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch121-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch121-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch121-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch121-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 121 replaces the full source tree. The server is already on Batch 120,
# so reject any hotfix made to files this batch updates.
echo "== checking live batch-119 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 120: $relative. Merge the server-side change before deploying batch 121."
  fi
done <<'BATCH120_BASELINE_HASHES'
3c78bef33cacb8c7656c9f07b7c6f8c75a90705bc2ee1af3ce38d73e837d8fdb src/app/globals.css
1f77c913ce1d0fc65247ca788439c65cb631537374daa9038f79c527094880b2 src/app/reports/page.tsx
BATCH120_BASELINE_HASHES
...
# The patient reports navigation was redesigned in Batch 121. Each report has
...
grep -q "SettingsSidebarLayout" "$STAGE_DIR/src/app/reports/page.tsx" || fail "Batch 121: reports side navigation is not wired."
for report_key in 'key: "ortho"' 'key: "referrals"' 'key: "duration"' 'key: "incomplete"'; do
  grep -q "$report_key" "$STAGE_DIR/src/app/reports/page.tsx" \
    || fail "Batch 121: a required patient report navigation item is missing: $report_key"
done
...
grep -q 'title="متابعة تقويم متأخرة"' "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 121: the overdue orthodontic follow-up report title is missing."
grep -q 'title="التحويلات"' "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 121: the referrals report title is missing."
