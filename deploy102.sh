...
ARCHIVE="clinic-app-batch102.tar.gz"
ARCHIVE_SHA256="80E5A90009E8C27A4BF14A62E6D3D37A547CE0134A9C9E30D5FAE8DC34FC1CF1"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch102-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch102-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch102-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch102-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch102-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 102 (security batch): every item below must actually be in the archive.
...
if ! grep -q 'DEFAULT_PASSWORD = "123"' "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 102: DEFAULT_PASSWORD constant is missing from src/lib/auth.ts (the change-password flow is not in this archive)."
fi
...
if ! grep -q "DEFAULT_PASSWORD_DEADLINE = \"2026-10-01\"" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 102: DEFAULT_PASSWORD_DEADLINE is missing from src/lib/auth.ts."
fi
if ! grep -q "must_change_password INTEGER NOT NULL DEFAULT 1" "$STAGE_DIR/src/lib/db/client.ts"; then
  fail "Batch 102: the must_change_password column migration is missing from src/lib/db/client.ts."
fi
if ! grep -q "getSessionSecret" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 102: the SESSION_SECRET check is missing from src/lib/auth.ts."
fi
...
for guarded in saveSettingsAction addServiceAction updateServiceAction; do
  if ! grep -A 8 "^export async function ${guarded}(" "$STAGE_DIR/src/app/settings/actions.ts" \
    ...
    fail "Batch 102: ${guarded} still has no admin check in src/app/settings/actions.ts."
  fi
done
if ! grep -q 'segments\[0\] === "hr-documents"' "$STAGE_DIR/src/app/patient-files/[...path]/route.ts"; then
  fail "Batch 102: the HR-document read guard is missing from the patient-files route."
fi
if ! grep -q "must_change_password" "$STAGE_DIR/src/lib/session.ts"; then
  fail "Batch 102: the per-request user re-check is missing from src/lib/session.ts."
fi
...
# Batch 102: audit-log coverage for the destructive/sensitive actions.
for pair in \
  ...
  file="${pair%%:*}"
  event="${pair##*:}"
  grep -q "$event" "$STAGE_DIR/$file" || fail "Batch 102: audit event '$event' is missing from $file."
done
grep -q "logAdminEvent" "$STAGE_DIR/src/lib/audit.ts" || fail "Batch 102: logAdminEvent helper is missing from src/lib/audit.ts."
grep -q "CLINIC_BACKUP_UPLOADS" "$STAGE_DIR/scripts/backup.sh" || fail "Batch 102: the database-only backup switch is missing from scripts/backup.sh."

# Batch 102 (was 96 for HR/expenses): every form screen must tell the user why
...
for helper in "src/app/hr/actions.ts:hrError" "src/app/expenses/actions.ts:expenseError" \
  ...
  file="${helper%%:*}"
  name="${helper##*:}"
  grep -q "function $name" "$STAGE_DIR/$file" || fail "Batch 102: the $name helper is missing from $file."
done
for pair in "src/app/hr/page.tsx:HR_ERRORS" "src/app/expenses/page.tsx:EXPENSE_ERRORS" \
  ...
  file="${pair%%:*}"
  name="${pair##*:}"
  grep -q "$name" "$STAGE_DIR/$file" || fail "Batch 102: the $name message map is missing from $file."
done
...
for target in src/app/hr/actions.ts src/app/expenses/actions.ts src/app/settings/actions.ts src/app/inventory/actions.ts; do
  ...
  if (( leftover != guards )); then
    fail "Batch 102: $target still has $(( leftover - guards )) silent return(s) that give the user no message."
  fi
done

# Batch 102: reports screen reorganised into three tabs, system log report added,
...
grep -q "view_system_log" "$STAGE_DIR/src/lib/permission-defs.ts" || fail "Batch 102: the view_system_log permission is missing from permission-defs.ts."
grep -q "renderReportTabs" "$STAGE_DIR/src/app/reports/page.tsx" || fail "Batch 102: the reports tab layout is missing from src/app/reports/page.tsx."
grep -q 'only={\["unpaid"\]}' "$STAGE_DIR/src/app/reports/page.tsx" || fail "Batch 102: the unpaid report did not move to the financial tab."
...
if grep -q "audit_log" "$STAGE_DIR/src/app/invoices/page.tsx"; then
  fail "Batch 102: the invoices screen still queries audit_log; the section should have moved to the reports system tab."
fi
...
grep -q "delete_patient" "$STAGE_DIR/src/app/patients/actions.ts" || fail "Batch 102: patient deletion is still not recorded in the audit log."
if ! awk '/const patientRow = await db/{before=NR} /deleteFrom\("patients"\)/{if(!before||NR<before) exit 1} END{exit !before}' \
  ...
  fail "Batch 102: the patient details must be read BEFORE the delete transaction - after it there is nothing left to read."
fi
...
# --------------------------------------------------------------------------
# Batch 102: the fix for the crash batch 98 shipped, plus the check that stops
...
grep -q 'from "@/lib/system-log-labels"' "$STAGE_DIR/src/components/SystemLogReport.tsx" \
  || fail "Batch 102: SystemLogReport.tsx does not import the labels from @/lib/system-log-labels - the crash fix is not in this archive."
if grep -qE '^export (const|let|var|default|function) ' "$STAGE_DIR/src/app/reports/system-log-actions.ts"; then
  fail "Batch 102: system-log-actions.ts still has a non-async export - this is exactly what crashed /reports in batch 98."
fi
...
node "$STAGE_DIR/scripts/check-use-server.mjs" "$STAGE_DIR/src" \
  || fail "Batch 102: a \"use server\" file exports something other than an async function. It would build fine and then crash the page in the browser. Deploy stopped."

# --------------------------------------------------------------------------
# Batch 102: patient duration-in-clinic report (attended_at -> completed_at),
...
grep -q "view_patient_duration_report" "$STAGE_DIR/src/lib/permission-defs.ts" \
  || fail "Batch 102: the view_patient_duration_report permission is missing from permission-defs.ts."
grep -q "PatientDurationReport" "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 102: the patient duration report is not wired into src/app/reports/page.tsx."
grep -q "missingAttendedAt" "$STAGE_DIR/src/app/reports/patient-duration-actions.ts" \
  || fail "Batch 102: the missing-attended_at count is missing from patient-duration-actions.ts - averages would look precise when part of the data can't be measured."
...
if grep -qE '^export (const|let|var|default|function) ' "$STAGE_DIR/src/app/reports/patient-duration-actions.ts"; then
  fail "Batch 102: patient-duration-actions.ts has a non-async export - this is the same mistake that crashed /reports in batch 98."
fi
...
grep -q "patientQuery" "$STAGE_DIR/src/app/reports/patient-duration-actions.ts" \
  || fail "Batch 102: the patient name/file-number search filter is missing from patient-duration-actions.ts."
grep -q "patientQuery" "$STAGE_DIR/src/components/PatientDurationReport.tsx" \
  || fail "Batch 102: the patient search input is missing from PatientDurationReport.tsx."
...
grep -q "defaultOpen" "$STAGE_DIR/src/components/WhatsAppOperationalReports.tsx" \
  || fail "Batch 102: the defaultOpen prop is missing from WhatsAppOperationalReports.tsx."
if ! grep -q 'only={\["ortho", "referrals"\]} defaultOpen' "$STAGE_DIR/src/app/reports/page.tsx"; then
  fail "Batch 102: the patients-tab operational reports panel is not set to open by default in src/app/reports/page.tsx."
fi
...
grep -q "title?: string" "$STAGE_DIR/src/components/WhatsAppOperationalReports.tsx" \
  || fail "Batch 102: the title prop is missing from WhatsAppOperationalReports.tsx."
grep -q 'only={\["unpaid"\]} title="تم بدون دفع"' "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 102: the financial-tab panel is not titled 'تم بدون دفع' in src/app/reports/page.tsx."
grep -q 'title="تقرير المتابعات والتحويلات"' "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 102: the patients-tab panel is not titled 'تقرير المتابعات والتحويلات' in src/app/reports/page.tsx."

# --------------------------------------------------------------------------
# Batch 102: patient-name autocomplete on the duration report (type-to-search
...
grep -q "getPatientDurationPatientsAction" "$STAGE_DIR/src/app/reports/patient-duration-actions.ts" \
  || fail "Batch 102: getPatientDurationPatientsAction is missing from patient-duration-actions.ts - the autocomplete has nothing to search."
grep -q "getPatientDurationPatientsAction" "$STAGE_DIR/src/components/PatientDurationReport.tsx" \
  || fail "Batch 102: PatientDurationReport.tsx does not load the patient list - the autocomplete dropdown was not wired in."
grep -q "patientSuggestions" "$STAGE_DIR/src/components/PatientDurationReport.tsx" \
  || fail "Batch 102: the autocomplete suggestions dropdown is missing from PatientDurationReport.tsx."
grep -q "isNewlyDone" "$STAGE_DIR/src/app/appointments/billing-actions.ts" \
  || fail "Batch 102: the completed_at fix on the payment-collection path is missing from billing-actions.ts."
...
grep -q "CLINIC_TIMEZONE" "$STAGE_DIR/src/components/PatientDurationReport.tsx" \
  || fail "Batch 102: PatientDurationReport.tsx computes 'today' without the clinic timezone - this is the batch-28 UTC-vs-Cairo bug again."
...
if grep -vE '^\s*//' "$STAGE_DIR/src/components/PatientDurationReport.tsx" | grep -q "toISOString"; then
  fail "Batch 102: PatientDurationReport.tsx still uses toISOString() for a date - that reads UTC, not clinic-local time."
fi
...
echo
echo "Batch 102 adds the patient duration-in-clinic report, plus small"
echo "follow-ups to the reports screen."
echo
echo "Check it yourself now:"
echo "  1) Open the reports screen, patients tab."
echo "  2) The panel that used to say 'التقارير التشغيلية عند الطلب' is now"
echo "     named 'تقرير المتابعات والتحويلات', and it's open by default -"
echo "     no extra click needed."
echo "  3) Financial tab: the same panel there is now titled 'تم بدون دفع'"
echo "     directly, since it only ever held that one report."
echo "  4) Back in the patients tab, scroll to 'مدة المريض في العيادة'."
echo "     Start typing a patient's name or file number - a dropdown of"
echo "     matching patients now appears as you type (same as the booking"
echo "     box in the central schedule); click one to fill the box, or"
echo "     leave it empty for everyone. Pick a date range (or 'كل الفترة')"
echo "     and press the show button."
echo "  5) A real bug fix: appointments completed through the normal payment-"
echo "     collection flow now record completed_at, so they will show up"
echo "     with a real duration going forward. Appointments completed before"
echo "     this deploy will still show as 'بدون وقت وصول مسجل' - that is"
echo "     expected, the data was never recorded for them."
echo "  6) Another real bug fix, found from your own test just now: 'كل"
echo "     الفترة' was computing 'today' in UTC instead of Cairo time, so"
echo "     testing it between midnight and ~2-3am Cairo time could silently"
echo "     drop that day's appointments from the range. Fixed - test data"
echo "     dated today should show up now."
echo
