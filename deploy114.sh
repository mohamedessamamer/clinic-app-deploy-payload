...
ARCHIVE="clinic-app-batch114.tar.gz"
ARCHIVE_SHA256="365006BC4B62163D45C083CA0C44D4F11844AA78F71C1F5206076F5508134012"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch114-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch114-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch114-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch114-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch114-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 114 replaces the full source tree. The server is already on Batch 113,
# so reject any hotfix made to the files this batch updates.
echo "== checking live batch-113 chart source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 113: $relative. Merge the server-side change before deploying batch 114."
  fi
done <<'BATCH114_BASELINE_HASHES'
2b8ad2d33455a8f736fc3a8ebf8f0bdea8848ad4c0db48a2833bd0f9cd775932 src/app/favicon.ico
470250ceb39e6953140ef730cb080c6506153a26f4742133b010f74fd692f008 src/app/layout.tsx
d9a70f9c23112c6ee67e79c4c6a1b4c68e9f5fd9f9631af2c9c661bf947b418a src/app/globals.css
d9118c8e2de1406ff188a1c105c845fa3a3a70809f8bfaae571b3aac786a29b9 src/components/ortho/InteractiveToothChart.tsx
BATCH114_BASELINE_HASHES
...
# Batch 114: place the interactive-chart editor in the lower white space and
# replace the legacy browser favicon with the compact clinic logo.
grep -q 'top:26rem' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 114: chart summary editor is not below the lower teeth."
...
grep -q 'font-size:.94rem' "$STAGE_DIR/src/app/globals.css" \
  || fail "Batch 114: chart summary text was not enlarged."
