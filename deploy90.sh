...
ARCHIVE="clinic-app-batch90.tar.gz"
ARCHIVE_SHA256="3080F778C93BE82322D9E84FAC186B46A4B09DF49B30658A7709D300B0F5565D"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch90-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch90-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch90-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch90-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch90-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 90: admin-only permission to wipe expenses-screen test data, and
