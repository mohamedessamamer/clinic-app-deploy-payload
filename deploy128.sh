...
ARCHIVE="clinic-app-batch128.tar.gz"
ARCHIVE_SHA256="BC1CB89544A6D4716F255894A3DA1645D960CB71B30BAD0F658640D1C0CCF8E8"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch128-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch128-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch128-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch128-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch128-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 128 upgrades the published Batch 127 supplied by the owner.
# Refuse to overwrite unreviewed server-side changes before stopping the app.
echo "== checking live batch-127 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from published batch 127: $relative. Merge the server-side change before deploying batch 128."
  fi
done <<'BATCH127_BASELINE_HASHES'
fa7dc34e2c8b95b931f25720347a2c0cc856bf5eba3f68cbcb43dbadf9dddff9 src/app/api/patient-files/upload/route.ts
982698021e2485beebd4ba8fa68de0e8f6165edc8812eda012665b8aed8fa2ff src/app/patients/[id]/actions.ts
a2e06d4f9c02066dfa9dd953e238937c3e82ffca6793f8d23f6e387976f82b62 src/app/patients/[id]/billing/page.tsx
c2a8bacc55018be4892712898ca592254f7e799724e42a8ca9a58072fd4b372b src/app/patients/[id]/images/template-actions.ts
...
6951c2ebe73e42edde7c1ee29153668202fd5d7cf15580b6ebb307a8a23d779a src/components/ortho/ImageTemplateBoard.tsx
...
BATCH127_BASELINE_HASHES
require_absent "$APP_DIR/src/lib/images/hash.ts"
require_absent "$APP_DIR/src/lib/images/orientation.ts"
...
require_file "$STAGE_DIR/docs/BATCH128.md"
...
# --------------------------------------------------------------------------
# Regression guard for the crash the FIRST batch128 deploy attempt hit live:
...
# deploy128.sh correctly rolled back. Fixed by moving that CREATE INDEX into
...
POST_SWAP_HASHES_FILE="$STAGE_DIR/.deploy128-expected-hashes"
...
LIVE_HASHES_FILE="$APP_DIR/.deploy128-expected-hashes"
[[ -f "$LIVE_HASHES_FILE" ]] || fail "Post-swap verification snapshot did not survive the swap (.deploy128-expected-hashes missing) — aborting rather than guessing."
...
while IFS= read -r line; do
  ...
  if [[ ! -f "$live_file" ]]; then
    echo "[deploy128] MISMATCH: expected file missing after swap: $rel" >&2
    POST_SWAP_MISMATCH=1
    continue
  fi
  live_hash="$(sha256sum "$live_file" | cut -d' ' -f1)"
  if [[ "$live_hash" != "$expected_hash" ]]; then
    echo "[deploy128] MISMATCH: live file does not match what was just built/tested: $rel" >&2
    POST_SWAP_MISMATCH=1
  fi
done < "$LIVE_HASHES_FILE"
rm -f "$LIVE_HASHES_FILE"
if (( POST_SWAP_MISMATCH != 0 )); then
  fail "Post-swap verification failed — the live app does not match the batch128 archive. Rolling back automatically instead of serving unverified files."
fi
...
echo
echo "== Batch 128 =="
echo "Batch 128: upgrades published Batch 127; automatic/manual rotation and template lightbox updates."
