...
ARCHIVE="clinic-app-batch89.tar.gz"
ARCHIVE_SHA256="DEAE4A1775A88ABBA136E56716535EE00165DFA7DB453F92CC75F6A8CBBD5B05"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch89-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch89-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch89-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch89-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch89-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 89: admin-only permission to wipe expenses-screen test data, and
