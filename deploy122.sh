...
ARCHIVE="clinic-app-batch122.tar.gz"
ARCHIVE_SHA256="3A2601F48997F519B66C49799FB15B9B0A2323555F33271ED977B15363B9848E"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch122-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch122-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch122-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch122-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch122-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 122 replaces the full source tree. The server is already on Batch 121,
# so reject any hotfix made to files this batch updates.
echo "== checking live batch-121 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 121: $relative. Merge the server-side change before deploying batch 121."
  fi
done <<'BATCH121_BASELINE_HASHES'
3b8064798c7d78f86fec40854e422cde1a08463d8db95cffa04e9e24ca023289 src/app/globals.css
70e2ffeaf6a347e28345bc4b1c3320f1a0085aebb88b69fd4aa8e70565420f32 src/app/page.tsx
be52ded7e808e4eb9dcde770308fe8ac3f11ee9d49941385e8c2616a1a913072 src/app/appointments/actions.ts
c927384bde6b17d67435f0f9acb9090b002a67f018aee0bae8d14df0ee9cf6dd src/app/front-desk/page.tsx
2a8e9a5343a300c1b5f21bb7bdc70e12511a5fba2d283a1e3b50032a1cba143c src/app/reports/page.tsx
f73653ed7974ff6ef8933b6745d67acd43b7001512433ac4d84b4bc34975adb9 src/components/CentralSchedule.tsx
d6f15ef52e1b80d9fa167e3435c9df3a6eff5325eca263cc3e83ce2d49ea0bb1 src/components/ReportsFilterForm.tsx
3b0811ef3ece5d71c3c454fbbc84686a337e7f3a369abb3a434d17422a80e08a src/lib/reception-wait.ts
653a4e2f78575a7e0f7a70438b5409df95eee97e3a6b51713c45378e6a4ec22f src/lib/db/client.ts
BATCH121_BASELINE_HASHES
...
echo
echo "== Batch 122 =="
echo "Home timeline cards and shared appointment actions; doctor-only appointment list without shifts."
echo "Doctor totals hidden; dashboard totals permission retained; stacked report navigation."
