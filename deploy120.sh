...
ARCHIVE="clinic-app-batch120.tar.gz"
ARCHIVE_SHA256="9517A15E805BF9CE26DC8D01D9FAA781F8DFA993155C131713DCEAC3411C22DD"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch120-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch120-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch120-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch120-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch120-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 120 replaces the full source tree. The server is already on Batch 119,
...
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 119: $relative. Merge the server-side change before deploying batch 120."
  fi
done <<'BATCH120_BASELINE_HASHES'
c1eafa7b906cb7609dfbf7bbe6b15d8d589413b549616ab5b3b38d6cd65278e9 src/app/globals.css
ad7ec0c86276a35d59d260e701a3dd87a794b4b0a014ca533777cf53e2a792f9 src/app/reports/page.tsx
BATCH120_BASELINE_HASHES
[[ ! -e "$APP_DIR/src/components/ReportSubnav.tsx" ]] || fail "Live source already contains ReportSubnav. Merge it before deploying batch 120."
