...
ARCHIVE="clinic-app-batch92.tar.gz"
ARCHIVE_SHA256="D3C0578B5322C61A15987BAF45BCCF5567B70684369EC531E61FEFC7C0C0AB06"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch92-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch92-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch92-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch92-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch92-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
