...
ARCHIVE="clinic-app-batch131.tar.gz"
ARCHIVE_SHA256="2EDB36F8151C7206445F51301A4BD148EF82EFFE68187C744441E8C924C7E279"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch131-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch131-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch131-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch131-deploy.XXXXXX)"
...
# Batch 131 upgrades the published Batch 130 (== the deploy that shipped
# settings sections + custom image kinds, patient avatar on schedule cards,
# the avatar-crop crash fix, and opt-in conservative learning).
# Refuse to overwrite unreviewed server-side changes before stopping the app.
#
# These baseline hashes were recomputed directly from the actual uploaded
# clinic-app-batch130.tar.gz (not hand-copied from an older script) —
# covering the whole ortho-image-feature surface (same set batch130 checked)
# plus every file batch131 itself touches (PatientTaskButton.tsx). As with
# every previous batch, spot-check a couple of these against the live server
# yourself before running this for real:
#   sha256sum /opt/clinic-app/src/components/CentralSchedule.tsx
#   sha256sum /opt/clinic-app/src/lib/settings.ts
# If either does not match the values below, STOP and tell us — it would
# mean the live server is not actually running batch130 as published.
echo "== checking live batch-130 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from published batch 130: $relative. Merge the server-side change before deploying batch 131."
  fi
done <<'BATCH130_BASELINE_HASHES'
...
8c64e5855b13b7faae069fba6743fb742fc32420237223a406fb4d8c8cba43f7 src/lib/images/classify.ts
...
ffbbd78ab27f57f7fb930ee859607e5362d25d6f0b84ef59c65ef5ced370e520 src/app/globals.css
a5f6708932f35b6ba8729392c1acc73d6a51a76c8a221b76a6fa578267a32d7a src/components/PatientTaskButton.tsx
...
POST_SWAP_HASHES_FILE="$STAGE_DIR/.deploy131-expected-hashes"
...
LIVE_HASHES_FILE="$APP_DIR/.deploy131-expected-hashes"
[[ -f "$LIVE_HASHES_FILE" ]] || fail "Post-swap verification snapshot did not survive the swap (.deploy131-expected-hashes missing) — aborting rather than guessing."
...
echo "== verifying Batch 112 through 130 database changes =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 112-130 database verification failed."
