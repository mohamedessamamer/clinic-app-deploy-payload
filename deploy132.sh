...
ARCHIVE_SHA256="E4A0F1F359679FECA1DA1D487A8BEFCDEB2B657F9A16DBBADB28A05865FBAC6D"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch132-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch132-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch132-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch132-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch131-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 132 upgrades the published Batch 131 (== the deploy that shipped the
# Task popup position fix, the avatar-crop centering fix, and the ortho chart
# tablet layout fix).
# Refuse to overwrite unreviewed server-side changes before stopping the app.
#
# These baseline hashes were computed directly from the actual uploaded
# clinic-app-batch131.tar.gz. The list covers everything batch131 checked PLUS
# every file batch132 itself touches, so a server-side edit to any of them is
# caught before the app is stopped.
#
# IMPORTANT: if this step fails on almost every file at once, the most likely
# cause is NOT a server-side edit — it is that batch131 was never actually
# deployed and the live server is still on batch130. STOP and say so; the fix
# is a regenerated baseline, not forcing past this check.
#   sha256sum /opt/clinic-app/src/components/PatientTaskButton.tsx
#   -> b7f9961db227d0d6a91264f1d5db8c629cb7edd88c567430e9bbbca938dafa70 on batch131
#   -> a5f6708932f35b6ba8729392c1acc73d6a51a76c8a221b76a6fa578267a32d7a on batch130
echo "== checking live batch-131 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from published batch 131: $relative. Either a server-side change needs merging, or batch131 was never deployed (see the note above this check). Do not force past this."
  fi
done <<'BATCH131_BASELINE_HASHES'
05f83df8049b94026803cc72047b2e1ac783d0415ca156ec7a1690b57505bd31 src/app/api/patient-files/upload/route.ts
5331786ef491671837111f95da36c6a5c4bca0acc7b85d9e4aaeb150711052d3 src/app/patients/[id]/actions.ts
a2e06d4f9c02066dfa9dd953e238937c3e82ffca6793f8d23f6e387976f82b62 src/app/patients/[id]/billing/page.tsx
8947cc1a42c1dee968807d92be0cc8ddd502be828c3d6719f57974931d10742c src/app/patients/[id]/images/template-actions.ts
...
2322a67bb60d0e113a7c856f84ffce0795e3d9386d059d75425a944222af0d0e src/components/ortho/ImageTemplateBoard.tsx
48fa5f38b6a0a2a95c84ac487623c0fd740909c4cb443b0cbd70344bc06f3cb7 src/components/ortho/OrthoAddImageGroupForm.tsx
b49654bbea5f977f7d62d1b48b9c606fe997daf4e3b3b178f04db2af331f99ff src/components/ortho/OrthoImageGallery.tsx
81aa05ffa2b6f2390b6f8e0c6a53ecdaba940de5b7892e3d740e7efeb4be8042 src/components/ortho/OrthoChartV2.tsx
30c6346b51e4644c9b4e93c0d495354c4242114fd4d2a50b0fbae1648aa1dcca src/lib/db/client.ts
81be4bd3739ca919366ae3d4dbf4c4141ae16c61126240ad910f3425b291a4b7 src/lib/db/schema.sql
9ab2b8649b9d2c66a781a47e08d60cf1513acdaf75c0177b63bcaab94a530ddb src/lib/db/types.ts
1af461ecf1d542ab000bd95d19f493cda0b0fad37a2ebf8d249d2363f0a8705f src/lib/images/classify.ts
...
0b9d361a356099ba28bf46d6fa0b3906732ee83d0630f1acf6c10159337031d2 src/lib/images/orientation.ts
...
67c8aac88b75b8f7769cfd1fad2720ffe9ba977a1ef3e732a9df8663adf989b4 src/app/patient-files/[...path]/route.ts
a4d6698654204459ba4af9226b619ded05d6d4b08de9bfd54f9e4515b96ef3c6 src/lib/settings.ts
f19c3a758d629695b581a2ebfc61fa7acfdffa59e7ffcebfac50969094b08bad src/lib/images/template.ts
...
da7ef4850969e3b7a2d25f6dc1324614bdec0bc5f4dbbbae7c27b6a3e7da6d28 src/components/OrthoImageTemplateForm.tsx
b5d13bc10c149dbbcf38bcbb6bcd5f4fda837d1c39a2df5e068808650a47cc86 src/app/front-desk/page.tsx
f8e98ca68f04032e4488f5c2c55606e4d484b61595d91fbd77883da6a7f6bedf src/components/CentralSchedule.tsx
d4fad49d4e631d3c9c7ac7b38475d785692f4607475813cfc0ee5edda359e65e src/app/globals.css
b7f9961db227d0d6a91264f1d5db8c629cb7edd88c567430e9bbbca938dafa70 src/components/PatientTaskButton.tsx
71aeb44454725cdc737b021cf100e36ad8690371b4b89e7cd80246556991cd5a src/lib/images/custom-kinds.ts
fc890653189fb8e7a2d16b93f67598b580ebec63970ed97a83eaf09df85e069e src/lib/images/learned-overrides.ts
1b84d2bd1521cbc61b4f4198485f8deea85de730d76054447789728bf43b3a91 src/app/reports/page.tsx
c930cce3b8643c47e9f490de9ffac10851296667f927dfeaca024813c5d1fa75 src/lib/hr-payroll.ts
bf05d96d17faab66ff47105daa6467b9a09484e1c7f7ebe771268549e1ad917d src/lib/hr-payroll-math.ts
154e1a8f8fc1e6f0098b29f2a2ec1875d1dca84fc79c33aeec33979b57e0ecc7 src/lib/permission-defs.ts
0c448a6dd4018971739f8d27efb2d9f0e13a1b0b9f169a837ce97dde20502fa3 src/components/HrWorkspace.tsx
a7a4bf5fd14de0fcba176af74b6db8f033446ea57dd5ade0f04a89b24ef10daf src/components/HrPayrollTable.tsx
608b1310db30da6aece035329ee8f76ec87159725c616616c795cd125308cc02 src/app/expenses/page.tsx
106f3603e1da4c9eede3e63a5f3e0ed1f4e64b00a2c2f45bb22683d21de6be1c src/app/expenses/actions.ts
40a5f0d4005969c7233da050b84f7457322fec060be8bd232bc59dbdd4be16db src/app/hr/actions.ts
3ed3a884c777a3fce76137f8e453a9f6cfebe25e15e49fc186bfeea50619f838 scripts/import-legacy-visits.mjs
BATCH131_BASELINE_HASHES
...
require_file "$STAGE_DIR/docs/BATCH132.md"
require_file "$STAGE_DIR/src/components/HrPayrollTable.tsx"
require_file "$STAGE_DIR/src/lib/hr-payroll.ts"
require_file "$STAGE_DIR/src/lib/hr-payroll-math.ts"
...
POST_SWAP_HASHES_FILE="$STAGE_DIR/.deploy132-expected-hashes"
...
LIVE_HASHES_FILE="$APP_DIR/.deploy132-expected-hashes"
[[ -f "$LIVE_HASHES_FILE" ]] || fail "Post-swap verification snapshot did not survive the swap (.deploy132-expected-hashes missing) — aborting rather than guessing."
...
echo "== verifying Batch 112 through 132 database changes =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 112-132 database verification failed."
...
echo
echo "== Batch 131 =="
echo "Batch 131: upgrades published Batch 130. No database migration in this batch."
echo "1) Task popup on the patient page no longer opens off the right edge of the viewport."
echo "2) Avatar face-crop centers higher (toward the real face), instead of the raw skin-blob center"
echo "   which included neck/chest skin and pushed the crop down, leaving too much hair on top."
echo "3) Interactive orthodontic chart keeps its full working width at every screen size (tablet"
echo "   portrait included), with a horizontal scrollbar instead of squashing/overlapping its teeth,"
echo "   bulk-bonding badges, and header labels."
