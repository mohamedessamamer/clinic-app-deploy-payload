...
ARCHIVE="clinic-app-batch103.tar.gz"
ARCHIVE_SHA256="1B0225C57F6F4227AB0F3FAADFD42B52CC10E4B506D6B07CB75556CD4D277EAE"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch103-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch103-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch103-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch103-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch103-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
echo
echo "Batch 103 adds the patient duration-in-clinic report, plus small"
...
echo "  7) Internal only, nothing to click: a migrations ledger table"
echo "     (schema_migrations) was added. Every database change from now on"
echo "     gets a name and a timestamp you can query directly on the server:"
echo "     sqlite3 /opt/clinic-app/data/clinic.db \\"
echo "       \"SELECT * FROM schema_migrations ORDER BY applied_at DESC;\""
echo "     Nothing about the existing 100+ startup migrations changed - they"
echo "     were already safe to re-run, so they were left alone."
echo
