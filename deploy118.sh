...
ARCHIVE="clinic-app-batch118.tar.gz"
ARCHIVE_SHA256="F3BFA58B4385E7E2B43A8CF8DB93A46D6D02029014FE1EC74CDD4F606F89F894"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch118-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch118-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch118-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch118-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch118-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 118 replaces the full source tree. The server is already on Batch 117,
# so reject any hotfix made to files this batch updates.
echo "== checking live batch-117 source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 117: $relative. Merge the server-side change before deploying batch 118."
  fi
done <<'BATCH118_BASELINE_HASHES'
b3f79d7f2f797987a23d2cab690aad67f00c894bc33d11e5ab1ebcf507b8a7cb src/app/page.tsx
1f0ca287f96d2206ccadcc1aa14b0e5c9f61d0577ada01169d48a21a59e7d5f7 src/app/globals.css
BATCH118_BASELINE_HASHES
[[ ! -e "$APP_DIR/src/components/HomeClinicStatus.tsx" ]] || fail "Live source already contains HomeClinicStatus. Merge it before deploying batch 118."
