...
ARCHIVE="clinic-app-batch126.tar.gz"
ARCHIVE_SHA256="BEB136A5EF76796A65F3BD1DBFFA71E40147314593DD4D2F5D116DA7CA61FDE4"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch126-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch126-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch126-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch126-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch126-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# دفعة 126 بتستبدل شجرة الكود كلها، وهي بتضم دفعة 124 كمان (اللي اتجهّزت
# ومااتنشرتش). السيرفر دلوقتي على Batch 123، فبنقارن بملفات 123 للملفات اللي
# الدفعة دي بتغيّرها بس — أي تعديل يدوي على السيرفر فيها بيوقف النشر.
...
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 123: $relative. Merge the server-side change before deploying batch 126."
  fi
done <<'BATCH123_BASELINE_HASHES'
37ddf2a6c9c22f8dcf7d36858050114ddd26c564c5eb170ebb61bfe3f4866828 next.config.ts
...
7bf7eb56a6734d91de3bf30eca1f335bf0070d2e6d1645ff7ab4a4c714d46584 src/app/patients/[id]/actions.ts
2612b4a7c12d09c5491ac402811a9858184e7b422e3d679ce851335f615a3d60 src/app/patients/[id]/orthodontics/page.tsx
...
fc09a93951469f51383498dc7c2fae3668e32f819ba81b2ec67f39eb02b90bd7 src/components/AddImageGroupForm.tsx
443a573250030f2b32f2d045161de55d700c6b9d7d18df30fbb07fb1b4e8b342 src/components/PatientImageGallery.tsx
fc6b925c71fcb4cd89afefa84c4be9e5641c116b726aeccc2e4dd1592ebe4166 src/components/ortho/OrthoAddImageGroupForm.tsx
bf6c5c84434ba23c74514f7a964dd6674e86c2f7c4cb06c5ac68d4bf07bc1c1e src/components/ortho/OrthoChartV2.tsx
...
echo "== verifying Batch 112 through 124 database changes =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 112-124 database verification failed."
