...
ARCHIVE="clinic-app-batch113.tar.gz"
ARCHIVE_SHA256="223EB822D93E008D1A77CD1DED861ED252461442100804D0E3944DC8F4ECFB2F"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch113-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch113-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch113-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch113-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch113-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 113 replaces the full source tree. The server is already on Batch 112,
# so reject any hotfix made to the files this batch updates.
echo "== checking live batch-112 chart source before replacement =="
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 112: $relative. Merge the server-side change before deploying batch 113."
  fi
done <<'BATCH113_BASELINE_HASHES'
c2fbf643eb8a92c3a7aa3198a851e63ff7ad29fd18a7bf0b3bed9fb72c8ccdc0 src/lib/settings.ts
23f8cb9265fb36fada33746b94b24351831586d306e7acca53ba69a2554c55ad src/lib/permission-defs.ts
...
6d0930c35ffd87a051272bbeb416f22482028e4a350005d327fd784d452c4653 src/app/settings/actions.ts
a8661c853c13177678b7e6e8bf9e58d7187a08150528e6ac9db39f5c50dcd211 src/app/settings/page.tsx
479d47957a7ad538961aa4fc28aee5d17b7ef0f02add353f93a2776b18177769 src/app/patients/[id]/orthodontics/ortho-v2-actions.ts
4ad1b3ac586b55b3a248f925a393dc331d1a59108e7c34d48d183bc4bf166d9b src/components/ortho/InteractiveToothChart.tsx
68e19c5619df2e2df6cc195cf9183819dea31c01cc32a37b129d2fb23caaa616 src/components/ortho/OrthoChartV2.tsx
bcbd49ffcced703d78565bc36738f44b9b552062289a3af10a9f301c47fabc71 src/components/ortho/DiagnosticChart.tsx
71a22f4869e34b5c198681d9410fe940f107ba29a643973a5e347ea57bb39cc5 src/app/patients/[id]/orthodontics/page.tsx
f1d38f89db95d4fb66d4dadf20586ee9372167f14e945d44881e61d428990f32 src/app/globals.css
BATCH113_BASELINE_HASHES
[[ ! -e "$APP_DIR/src/components/OrthoChartSummaryOptionsForm.tsx" ]] \
  || fail "Live source already contains an unexpected Ortho chart options form. Merge it before deploying batch 113."
...
echo "== verifying Batch 112 and 113 database changes =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 112/113 database verification failed."
...
if (!migration || setting?.value !== "0" || Number(tubeCells?.total) !== 8 || !orthoColumns.includes("diagnosis_summary") || foreignKeys.length !== 0) process.exit(1);
NODE
SUCCESS=1
