...
ARCHIVE="clinic-app-batch111.tar.gz"
ARCHIVE_SHA256="013551A267739DEB6C0FA77B89272233FDD0BEC71B5CE81C84D7CF0FDC229C53"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch111-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch111-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch111-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch111-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch111-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 111 replaces the full source tree. Refuse to overwrite any server-side
...
while read -r expected relative; do
  ...
  if [[ "$actual" != "$expected" ]]; then
    fail "Live source differs from batch 110: $relative. Merge the server-side change before deploying batch 111."
  fi
done <<'BATCH111_BASELINE_HASHES'
...
for relative in \
  ...
  src/lib/tasks.ts; do
  [[ ! -e "$APP_DIR/$relative" ]] || fail "Live source already contains Batch 111 file: $relative. Merge that server-side change first."
done
...
# Batch 111: inventory lock, per-tooth tube stock, actionable tasks, and
...
grep -q '111_tasks_inventory_switch_tube_stock' "$STAGE_DIR/src/lib/db/client.ts" \
  || fail "Batch 111: database migration is missing."
grep -q 'inventory_deductions_enabled' "$STAGE_DIR/src/lib/inventory-control.ts" "$STAGE_DIR/src/components/InventoryDeductionToggle.tsx" \
  || fail "Batch 111: inventory master switch is not wired end-to-end."
grep -q 'TUBE_6_TEETH = \[16, 26, 36, 46\]' "$STAGE_DIR/src/lib/ortho/tube-stock.ts" \
  || fail "Batch 111: Tube 6 cell mapping is wrong or missing."
grep -q 'TUBE_7_TEETH = \[17, 27, 37, 47\]' "$STAGE_DIR/src/lib/ortho/tube-stock.ts" \
  || fail "Batch 111: Tube 7 cell mapping is wrong or missing."
grep -q 'insertInto("tasks")' "$STAGE_DIR/src/app/referrals/actions.ts" \
  || fail "Batch 111: referrals are not being created as tasks."
grep -q 'autoCloseReferralTasksForAppointment' "$STAGE_DIR/src/app/appointments/actions.ts" "$STAGE_DIR/src/app/appointments/billing-actions.ts" \
  || fail "Batch 111: referral tasks are not linked to appointment completion."
grep -q 'canEditAppointmentSlot' "$STAGE_DIR/src/app/appointments/actions.ts" \
  || fail "Batch 111: server-side doctor schedule authorization is missing."
grep -q 'canEditSelectedDate && canEditSchedule' "$STAGE_DIR/src/app/page.tsx" \
  || fail "Batch 111: future schedule editing is not enabled in the doctor UI."
...
echo "== verifying Batch 111 database migration =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 111 database verification failed."
...
if (!migration || setting?.value !== "0" || Number(tubeCells?.total) !== 8 || foreignKeys.length !== 0) process.exit(1);
NODE
SUCCESS=1
