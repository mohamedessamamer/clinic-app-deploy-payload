...
ARCHIVE="clinic-app-batch97.tar.gz"
ARCHIVE_SHA256="DB1637665024905C2F6A1D42BD0032B70028B1F99A2E2803938E63580B6A4A3A"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch97-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch97-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch97-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch97-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch97-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 97 (security batch): every item below must actually be in the archive.
...
if ! grep -q 'DEFAULT_PASSWORD = "123"' "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 97: DEFAULT_PASSWORD constant is missing from src/lib/auth.ts (the change-password flow is not in this archive)."
fi
...
if ! grep -q "DEFAULT_PASSWORD_DEADLINE = \"2026-10-01\"" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 97: DEFAULT_PASSWORD_DEADLINE is missing from src/lib/auth.ts."
fi
if ! grep -q "must_change_password INTEGER NOT NULL DEFAULT 1" "$STAGE_DIR/src/lib/db/client.ts"; then
  fail "Batch 97: the must_change_password column migration is missing from src/lib/db/client.ts."
fi
if ! grep -q "getSessionSecret" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 97: the SESSION_SECRET check is missing from src/lib/auth.ts."
fi
...
for guarded in saveSettingsAction addServiceAction updateServiceAction; do
  if ! grep -A 8 "^export async function ${guarded}(" "$STAGE_DIR/src/app/settings/actions.ts" \
    ...
    fail "Batch 97: ${guarded} still has no admin check in src/app/settings/actions.ts."
  fi
done
if ! grep -q 'segments\[0\] === "hr-documents"' "$STAGE_DIR/src/app/patient-files/[...path]/route.ts"; then
  fail "Batch 97: the HR-document read guard is missing from the patient-files route."
fi
if ! grep -q "must_change_password" "$STAGE_DIR/src/lib/session.ts"; then
  fail "Batch 97: the per-request user re-check is missing from src/lib/session.ts."
fi
...
# Batch 97: audit-log coverage for the destructive/sensitive actions.
for pair in \
  ...
  file="${pair%%:*}"
  event="${pair##*:}"
  grep -q "$event" "$STAGE_DIR/$file" || fail "Batch 97: audit event '$event' is missing from $file."
done
grep -q "logAdminEvent" "$STAGE_DIR/src/lib/audit.ts" || fail "Batch 97: logAdminEvent helper is missing from src/lib/audit.ts."
grep -q "CLINIC_BACKUP_UPLOADS" "$STAGE_DIR/scripts/backup.sh" || fail "Batch 97: the database-only backup switch is missing from scripts/backup.sh."

# Batch 97 (was 96 for HR/expenses): every form screen must tell the user why
...
for helper in "src/app/hr/actions.ts:hrError" "src/app/expenses/actions.ts:expenseError" \
  ...
  file="${helper%%:*}"
  name="${helper##*:}"
  grep -q "function $name" "$STAGE_DIR/$file" || fail "Batch 97: the $name helper is missing from $file."
done
for pair in "src/app/hr/page.tsx:HR_ERRORS" "src/app/expenses/page.tsx:EXPENSE_ERRORS" \
  ...
  file="${pair%%:*}"
  name="${pair##*:}"
  grep -q "$name" "$STAGE_DIR/$file" || fail "Batch 97: the $name message map is missing from $file."
done
...
for target in src/app/hr/actions.ts src/app/expenses/actions.ts src/app/settings/actions.ts src/app/inventory/actions.ts; do
  ...
  if (( leftover != guards )); then
    fail "Batch 97: $target still has $(( leftover - guards )) silent return(s) that give the user no message."
  fi
done
...
echo
echo "Batch 97 post-deploy steps:"
