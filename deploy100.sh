...
ARCHIVE="clinic-app-batch100.tar.gz"
ARCHIVE_SHA256="46F8F491C1F5407FCAFDE646EC35E65EDCB6D2F569D4037BB56DA84C5F40D214"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch100-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch100-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch100-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch100-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch100-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 100 (security batch): every item below must actually be in the archive.
...
if ! grep -q 'DEFAULT_PASSWORD = "123"' "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 100: DEFAULT_PASSWORD constant is missing from src/lib/auth.ts (the change-password flow is not in this archive)."
fi
...
if ! grep -q "DEFAULT_PASSWORD_DEADLINE = \"2026-10-01\"" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 100: DEFAULT_PASSWORD_DEADLINE is missing from src/lib/auth.ts."
fi
if ! grep -q "must_change_password INTEGER NOT NULL DEFAULT 1" "$STAGE_DIR/src/lib/db/client.ts"; then
  fail "Batch 100: the must_change_password column migration is missing from src/lib/db/client.ts."
fi
if ! grep -q "getSessionSecret" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 100: the SESSION_SECRET check is missing from src/lib/auth.ts."
fi
...
for guarded in saveSettingsAction addServiceAction updateServiceAction; do
  if ! grep -A 8 "^export async function ${guarded}(" "$STAGE_DIR/src/app/settings/actions.ts" \
    ...
    fail "Batch 100: ${guarded} still has no admin check in src/app/settings/actions.ts."
  fi
done
if ! grep -q 'segments\[0\] === "hr-documents"' "$STAGE_DIR/src/app/patient-files/[...path]/route.ts"; then
  fail "Batch 100: the HR-document read guard is missing from the patient-files route."
fi
if ! grep -q "must_change_password" "$STAGE_DIR/src/lib/session.ts"; then
  fail "Batch 100: the per-request user re-check is missing from src/lib/session.ts."
fi
...
# Batch 100: audit-log coverage for the destructive/sensitive actions.
for pair in \
  ...
  file="${pair%%:*}"
  event="${pair##*:}"
  grep -q "$event" "$STAGE_DIR/$file" || fail "Batch 100: audit event '$event' is missing from $file."
done
grep -q "logAdminEvent" "$STAGE_DIR/src/lib/audit.ts" || fail "Batch 100: logAdminEvent helper is missing from src/lib/audit.ts."
grep -q "CLINIC_BACKUP_UPLOADS" "$STAGE_DIR/scripts/backup.sh" || fail "Batch 100: the database-only backup switch is missing from scripts/backup.sh."

# Batch 100 (was 96 for HR/expenses): every form screen must tell the user why
...
for helper in "src/app/hr/actions.ts:hrError" "src/app/expenses/actions.ts:expenseError" \
  ...
  file="${helper%%:*}"
  name="${helper##*:}"
  grep -q "function $name" "$STAGE_DIR/$file" || fail "Batch 100: the $name helper is missing from $file."
done
for pair in "src/app/hr/page.tsx:HR_ERRORS" "src/app/expenses/page.tsx:EXPENSE_ERRORS" \
  ...
  file="${pair%%:*}"
  name="${pair##*:}"
  grep -q "$name" "$STAGE_DIR/$file" || fail "Batch 100: the $name message map is missing from $file."
done
...
for target in src/app/hr/actions.ts src/app/expenses/actions.ts src/app/settings/actions.ts src/app/inventory/actions.ts; do
  ...
  if (( leftover != guards )); then
    fail "Batch 100: $target still has $(( leftover - guards )) silent return(s) that give the user no message."
  fi
done

# Batch 100: reports screen reorganised into three tabs, system log report added,
...
grep -q "view_system_log" "$STAGE_DIR/src/lib/permission-defs.ts" || fail "Batch 100: the view_system_log permission is missing from permission-defs.ts."
grep -q "renderReportTabs" "$STAGE_DIR/src/app/reports/page.tsx" || fail "Batch 100: the reports tab layout is missing from src/app/reports/page.tsx."
grep -q 'only={\["unpaid"\]}' "$STAGE_DIR/src/app/reports/page.tsx" || fail "Batch 100: the unpaid report did not move to the financial tab."
...
if grep -q "audit_log" "$STAGE_DIR/src/app/invoices/page.tsx"; then
  fail "Batch 100: the invoices screen still queries audit_log; the section should have moved to the reports system tab."
fi
...
grep -q "delete_patient" "$STAGE_DIR/src/app/patients/actions.ts" || fail "Batch 100: patient deletion is still not recorded in the audit log."
if ! awk '/const patientRow = await db/{before=NR} /deleteFrom\("patients"\)/{if(!before||NR<before) exit 1} END{exit !before}' \
  ...
  fail "Batch 100: the patient details must be read BEFORE the delete transaction - after it there is nothing left to read."
fi
...
# --------------------------------------------------------------------------
# Batch 100: the fix for the crash batch 98 shipped, plus the check that stops
...
grep -q 'from "@/lib/system-log-labels"' "$STAGE_DIR/src/components/SystemLogReport.tsx" \
  || fail "Batch 100: SystemLogReport.tsx does not import the labels from @/lib/system-log-labels - the crash fix is not in this archive."
if grep -qE '^export (const|let|var|default|function) ' "$STAGE_DIR/src/app/reports/system-log-actions.ts"; then
  fail "Batch 100: system-log-actions.ts still has a non-async export - this is exactly what crashed /reports in batch 98."
fi
...
node "$STAGE_DIR/scripts/check-use-server.mjs" "$STAGE_DIR/src" \
  || fail "Batch 100: a \"use server\" file exports something other than an async function. It would build fine and then crash the page in the browser. Deploy stopped."

# --------------------------------------------------------------------------
# Batch 100: patient duration-in-clinic report (attended_at -> completed_at),
...
grep -q "view_patient_duration_report" "$STAGE_DIR/src/lib/permission-defs.ts" \
  || fail "Batch 100: the view_patient_duration_report permission is missing from permission-defs.ts."
grep -q "PatientDurationReport" "$STAGE_DIR/src/app/reports/page.tsx" \
  || fail "Batch 100: the patient duration report is not wired into src/app/reports/page.tsx."
grep -q "missingAttendedAt" "$STAGE_DIR/src/app/reports/patient-duration-actions.ts" \
  || fail "Batch 100: the missing-attended_at count is missing from patient-duration-actions.ts - averages would look precise when part of the data can't be measured."
...
if grep -qE '^export (const|let|var|default|function) ' "$STAGE_DIR/src/app/reports/patient-duration-actions.ts"; then
  fail "Batch 100: patient-duration-actions.ts has a non-async export - this is the same mistake that crashed /reports in batch 98."
fi
...
echo
echo "Batch 100 adds the patient duration-in-clinic report."
echo
echo "Check it yourself now:"
echo "  1) Open the reports screen, patients tab."
echo "  2) Pick a date range and press the show button."
echo "     A summary (average duration, per-doctor table) should appear,"
echo "     plus a count of appointments with no attended_at recorded."
echo
