...
ARCHIVE="clinic-app-batch105.tar.gz"
ARCHIVE_SHA256="55107E1C28AF01383D1074032724259D3B411C8E5BE325815FC2D9A9679828FE"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch105-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch105-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch105-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch105-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch105-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
echo
echo "Batch 105 adds the legacy visits/financials import tooling (data up to"
echo "2026-09-08). It changes NOTHING for existing users or existing data by"
echo "itself — the app behaves exactly as it did under batch 104 until you"
echo "run the import script yourself."
echo
