...
ARCHIVE="clinic-app-batch82.tar.gz"
ARCHIVE_SHA256="59A2041460FCEDF767283AEB9A3D49CA19FA2CFAC7E9705FE1D3CDD72CD4E501"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch82-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch82-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch82-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch82-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch82-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
test -f "$STAGE_DIR/src/lib/image-compress.ts"
...
test -f "$STAGE_DIR/src/components/HrRulesManager.tsx"
