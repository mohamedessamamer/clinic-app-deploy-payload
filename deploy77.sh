...
ARCHIVE="clinic-app-batch77.tar.gz"
ARCHIVE_SHA256="8FB74CC2D702CE0DABD646A68DC6E29514433CDFCF57A2C4C8FC8D2FF1E3CE16"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch77-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch77-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch77-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch77-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch77-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
test -f "$STAGE_DIR/src/app/referrals/actions.ts"
test -f "$STAGE_DIR/src/components/ReferralModal.tsx"
...
test -f "$STAGE_DIR/src/lib/whatsapp/appointment-reminders.ts"
test -f "$STAGE_DIR/src/instrumentation.ts"
test -f "$STAGE_DIR/src/app/whatsapp/actions.ts"
test -f "$STAGE_DIR/src/app/api/whatsapp/webhook/route.ts"
test -f "$STAGE_DIR/src/components/WhatsAppChat.tsx"
