...
ARCHIVE="clinic-app-batch88.tar.gz"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch88-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch88-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch88-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch88-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch88-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 88: admin-only permission to wipe expenses-screen test data, and
