...
ARCHIVE="clinic-app-batch96.tar.gz"
ARCHIVE_SHA256="A1313691AD674C46B95774FE3C8910C7B0785CAB5D080E1B7294E9A16B0CAC5C"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch96-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch96-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch96-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch96-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch96-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
# Batch 96 (security batch): every item below must actually be in the archive.
...
if ! grep -q 'DEFAULT_PASSWORD = "123"' "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 96: DEFAULT_PASSWORD constant is missing from src/lib/auth.ts (the change-password flow is not in this archive)."
fi
...
if ! grep -q "DEFAULT_PASSWORD_DEADLINE = \"2026-10-01\"" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 96: DEFAULT_PASSWORD_DEADLINE is missing from src/lib/auth.ts."
fi
if ! grep -q "must_change_password INTEGER NOT NULL DEFAULT 1" "$STAGE_DIR/src/lib/db/client.ts"; then
  fail "Batch 96: the must_change_password column migration is missing from src/lib/db/client.ts."
fi
if ! grep -q "getSessionSecret" "$STAGE_DIR/src/lib/auth.ts"; then
  fail "Batch 96: the SESSION_SECRET check is missing from src/lib/auth.ts."
fi
...
for guarded in saveSettingsAction addServiceAction updateServiceAction; do
  if ! grep -A 8 "^export async function ${guarded}(" "$STAGE_DIR/src/app/settings/actions.ts" \
    ...
    fail "Batch 96: ${guarded} still has no admin check in src/app/settings/actions.ts."
  fi
done
if ! grep -q 'segments\[0\] === "hr-documents"' "$STAGE_DIR/src/app/patient-files/[...path]/route.ts"; then
  fail "Batch 96: the HR-document read guard is missing from the patient-files route."
fi
if ! grep -q "must_change_password" "$STAGE_DIR/src/lib/session.ts"; then
  fail "Batch 96: the per-request user re-check is missing from src/lib/session.ts."
fi
...
# Batch 96: audit-log coverage for the destructive/sensitive actions.
for pair in \
  ...
  file="${pair%%:*}"
  event="${pair##*:}"
  grep -q "$event" "$STAGE_DIR/$file" || fail "Batch 96: audit event '$event' is missing from $file."
done
grep -q "logAdminEvent" "$STAGE_DIR/src/lib/audit.ts" || fail "Batch 96: logAdminEvent helper is missing from src/lib/audit.ts."
grep -q "CLINIC_BACKUP_UPLOADS" "$STAGE_DIR/scripts/backup.sh" || fail "Batch 96: the database-only backup switch is missing from scripts/backup.sh."

# Batch 96: no more silent validation failures on the HR and expenses forms.
grep -q "function hrError" "$STAGE_DIR/src/app/hr/actions.ts" || fail "Batch 96: hrError helper is missing from src/app/hr/actions.ts."
grep -q "function expenseError" "$STAGE_DIR/src/app/expenses/actions.ts" || fail "Batch 96: expenseError helper is missing from src/app/expenses/actions.ts."
grep -q "HR_ERRORS" "$STAGE_DIR/src/app/hr/page.tsx" || fail "Batch 96: the HR error message map is missing from src/app/hr/page.tsx."
grep -q "EXPENSE_ERRORS" "$STAGE_DIR/src/app/expenses/page.tsx" || fail "Batch 96: the expenses error message map is missing from src/app/expenses/page.tsx."
# Every remaining bare `return;` in these two files must be a permission guard
# (the page already blocks those users), never an unexplained validation exit.
for target in src/app/hr/actions.ts src/app/expenses/actions.ts; do
  leftover="$(grep -cE '\) return;[[:space:]]*$' "$STAGE_DIR/$target" || true)"
  guards="$(grep -cE '(canManageHr|canManageExpenses|!session)[^;]*\) return;[[:space:]]*$' "$STAGE_DIR/$target" || true)"
  if (( leftover != guards )); then
    fail "Batch 96: $target still has $(( leftover - guards )) silent validation return(s) that give the user no message."
  fi
done
...
echo
echo "Batch 96 post-deploy steps:"
