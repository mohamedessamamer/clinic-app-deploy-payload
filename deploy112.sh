...
ARCHIVE="clinic-app-batch112.tar.gz"
ARCHIVE_SHA256="E31FA2E547CD842CA4D5FA1F6CEE81730FD72A294DE6682930BB238F0EAABDA4"
...
BACKUP_DIR="/opt/clinic-app-data.backup-batch112-$STAMP"
PREVIOUS_DIR="/opt/clinic-app.previous-batch112-$STAMP"
FAILED_DIR="/opt/clinic-app.failed-batch112-$STAMP"
TEMP_DIR="$(mktemp -d /tmp/clinic-app-batch112-deploy.XXXXXX)"
...
cleanup() {
  ...
  if [[ -d "$TEMP_DIR" && "$TEMP_DIR" == /tmp/clinic-app-batch112-deploy.* ]]; then
    rm -rf -- "$TEMP_DIR"
  fi
...
echo "== verifying Batch 112 database migration =="
SUCCESS=0
CLINIC_DB_PATH="$APP_DIR/data/clinic.db" node - <<'NODE' \
  || fail "Batch 112 database verification failed."
